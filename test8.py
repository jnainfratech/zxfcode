...
def home():
    if request.method == 'POST':
        ...
        # Add the logo as a block reference
        insert_point = (0, 0)  # Replace with the desired insertion point
        scale_factor1 = length  # Replace with the desired scale factor
        scale_factor2 = width  # Replace with the desired scale factor
        
        rotation_angle = 0  # Replace with the desired rotation angle
        try:
           # Open the DXF file and get the block definition
            
            block = doc.blocks.get('hlh')

            # Get the model space and add the block reference
            msp = doc.modelspace()
            msp.add_blockref('hlh', insert_point, dxfattribs={
                'xscale': scale_factor1,
                'yscale': scale_factor2,
                'rotation': rotation_angle,
            })

        except ezdxf.DXFValueError as e:
            print(f'Error adding block reference: {e}')
        except ezdxf.DXFKeyError as e:
            print(f'Error getting block definition: {email}')
        except ezdxf.DXFTypeError as e:
            print(f'Error adding block reference: {e}')
